...
Public Class tempo
    ...
    'Shared ConnectionString As String = "Data Source=hp\espinho;Initial Catalog=Agenda;Integrated Security=True"
    ...
    Private Sub tempo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        datagridtempo.AllowUserToAddRows = False
        gbmsgtempo.Visible = False
    ...
    Private Sub btnsairtempo_Click(sender As Object, e As EventArgs) Handles btnsairtempo.Click
        ...
        If tipouser = 0 Then
            Bloquear_menu(False)
            formulario_principal.HorasToolStripMenuItem.Enabled = True
        Else
            Bloquear_menu(True)
        End If
    End Sub
    ...
    Private Sub txttempoencontrado_TextChanged(sender As Object, e As EventArgs) Handles txttempoencontrado.TextChanged
        Call conectar()
        Dim sql As String = "select * from Tempo " & " where intervaloHorario like '" & txttempoencontrado.Text & "%'"
        Dim sqlcom As New SqlCommand(sql, con)
        Dim dr As SqlDataReader = sqlcom.ExecuteReader()
        Dim n As Integer = dr.FieldCount
        Dim i As Integer = 0
