...
Public Class Registo
    ...
    Public Sub constregisto()
        Call conectar()
        Dim sql As String = "SELECT idregisto, idUtilizador,data,hora,viacomunicação,telefone,telemovel,qta,destinario,T.descricao,tiposerviço,A.anolectivo,obs  FROM Registo R, Anolectivo A, turma T  WHERE R.anoletivo=A.idAnolectivo and R.turma=T.idTurma"
        Dim sqlcom As New SqlCommand(sql, con)
        Dim dr As SqlDataReader = sqlcom.ExecuteReader
    ...
    Private Sub btneditaregisto_Click(sender As Object, e As EventArgs) Handles btneditaregisto.Click
        ...
        btnguardaregisto.Enabled = True
        btncancelaregisto.Enabled = True
        txtnutilizadoregistos.Text = datagridregistos.CurrentRow.Cells(0).Value
    ...
    Private Sub btneliminaregisto_Click(sender As Object, e As EventArgs) Handles btneliminaregisto.Click
        Try
            ...
            If resultado = DialogResult.Yes Then
                Call conectar()
                Dim sql As String = "delete from registos where  idregisto = '" & txtnumregistos.Text & "'"
                Dim sqlcom As New SqlCommand(sql, con)
                Dim dr As SqlDataReader = sqlcom.ExecuteReader
                Call desconectar()
    ...
    Private Sub txtnomeutilizadoregisto_TextChanged(sender As Object, e As EventArgs)
