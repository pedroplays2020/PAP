...
Public Class blocoSala
    ...
    Public Sub gribsala()
        Call conectar()
        Dim sql As String = "select * from sala"
        ...
        For k = 0 To n - 1
            campo = dr.GetName(k)
            datagridsala.Columns.Add(campo, campo)
        Next
        Dim i As Integer = 0
        While dr.Read
    ...
    Private Sub btneditarblocosala_Click(sender As Object, e As EventArgs) Handles btneditarblocosala.Click
        ...
        ElseIf tab = 2 Then
            ...
            comblocosala.SelectedItem = datagridsala.CurrentRow.Cells(2).ValueType
    ...
    Private Sub btneliminarblocosala_Click(sender As Object, e As EventArgs) Handles btneliminarblocosala.Click
        If tab = 1 Then
            Try
                ...
                If resultado = DialogResult.Yes Then
                    Call conectar()
                    Dim sql As String = "delete from bloco where idbloco = '" & txtnomebloco.Text & "'"
                    Dim sqlcom As New SqlCommand(sql, con)
                    Dim dr As SqlDataReader = sqlcom.ExecuteReader
                    Call desconectar()
        ...
        ElseIf tab = 2 Then
            Try
                btncancelarblocosala.Enabled = True
                txtnumsala.Text = DataGridBloco.CurrentRow.Cells(0).Value
                txtnomesala.Text = DataGridBloco.CurrentRow.Cells(1).Value
                Dim resultado = MsgBox("Tem a certeza que pretende elminar o registo? ", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "Eliminar sala")
                If resultado = DialogResult.Yes Then
                    Call conectar()
                    Dim sql As String = "delete from sala where idbsala = '" & txtnomesala.Text & "'"
                    Dim sqlcom As New SqlCommand(sql, con)
                    Dim dr As SqlDataReader = sqlcom.ExecuteReader
                    Call desconectar()
    ...
    Private Sub txtnomesala_TextChanged(sender As Object, e As EventArgs) Handles txtnomesala.TextChanged
        Call conectar()
        Dim sql As String = "select * from sala " & " where sala like '" & txtnomesala.Text & "%'"
        Dim sqlcom As New SqlCommand(sql, con)
        Dim dr As SqlDataReader = sqlcom.ExecuteReader()
        Dim n As Integer = dr.FieldCount
        Dim i As Integer = 0
