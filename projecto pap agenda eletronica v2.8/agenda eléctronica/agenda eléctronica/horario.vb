...
Public Class horario
    ...
    Public Sub lersala()
        ...
        Dim dt As DataTable = New DataTable
        dr.Fill(dt)
        comboblocohorario.ValueMember = "idsala"
        comboblocohorario.DisplayMember = "sala"
        comboblocohorario.DataSource = dt
        Call desconectar()
    End Sub
    ...
    Public Sub lerdias()
        ...
        Dim dt As DataTable = New DataTable
        dr.Fill(dt)
        comboblocohorario.ValueMember = "idDiaSemana"
        comboblocohorario.DisplayMember = "diaSemana"
        comboblocohorario.DataSource = dt
        Call desconectar()
    ...
    Public Sub lerhoras()
        Dim sql As String = "select idtempo, intervaloHorario fromn Tempo order by idtempo"
        Dim com As SqlCommand = New SqlCommand(sql, con)
        Dim dr As SqlDataAdapter = New SqlDataAdapter(com)
        Dim dt As DataTable = New DataTable
        dr.Fill(dt)
        comboblocohorario.ValueMember = "idtempo"
        comboblocohorario.DisplayMember = "intervaloHorario"
        comboblocohorario.DataSource = dt
        Call desconectar()
    End Sub
    ...
    Public Sub consulthorario()
        ...
        For k = 0 To n - 1
            'campo = dr.GetName(k)
            datagridhorario.Columns.Add("Código", "Descrição")
        Next
        Dim i As Integer = 0
        While dr.Read
        ...
        If botao = 1 Then
            ...
            If combodisciplinahorario.SelectedItem = -1 Then
                ...
                Else
                    ...
                    Call conectar()
                    Dim sql As String = "insert into Horario values ('" & RTrim(combareahorario.SelectedValue) & "', '" & combodisciplinahorario.SelectedValue & "', '" & comboblocohorario.SelectedValue & "', '" & combosalahorario.SelectedValue & "', '" & combodiashorario.SelectedValue & "', '" & combohorashorario.SelectedValue & "')"
                    Dim sqlcom As New SqlCommand(sql, con)
                    Dim dr As SqlDataReader = sqlcom.ExecuteReader
                    Call desconectar()
                    consulthorario()
            ...
            ElseIf botao = 2 Then
                Call conectar()
                'Dim sql1 As String = "update horario  set ??  = '" & txtnomebloco.Text & "' where idbloco= '" & txtbloconum.Text & "'"
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles txtconsultahorario.TextChanged
        Call conectar()
        '  Dim sql As String = "select * from  Horario " & " where  ?? like '" & txtconsultahorario.Text & "%'"
        Dim sqlcom As New SqlCommand(Sql, con)
        Dim dr As SqlDataReader = sqlcom.ExecuteReader()
        Dim n As Integer = dr.FieldCount
        Dim i As Integer = 0
