...
Public Class turma
    ...
    Public Sub consturma()
        Call conectar()
        Dim sql As String = "select T.idTurma, T.descricao, A.anolectivo from turma T, Anolectivo A where  T.idAnolectivo = A.idAnolectivo"
    ...
    Public Sub limparturma()
        ...
        txtnometurma.Clear()
        combanoturma.SelectedValue = -1
        combotipocursoturma.SelectedItem = -1
        comboanoletivoturma.SelectedValue = -1
    End Sub
    ...
    Private Sub txtnometurma_TextChanged(sender As Object, e As EventArgs) Handles txtnometurma.TextChanged
        ...
        Dim sql As String = "select  T.idTurma, T.descricao, A.anolectivo from turma T, Anolectivo A " & " where T.descricao  like '" & txtnometurma.Text & "%' and T.idAnolectivo = A.idAnolectivo"
        Dim sqlcom As New SqlCommand(sql, con)
        Dim dr As SqlDataReader = sqlcom.ExecuteReader()
        Dim n As Integer = dr.FieldCount
        Dim i As Integer = 0
